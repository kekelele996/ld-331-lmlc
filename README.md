...
- **调班与替班**：员工提交双方班次的调换申请后，先由被指定的替班人确认——同意才进入主管审批，拒绝则申请直接结束；主管通过时在一个数据库事务中原子交换双方排班，全程记录申请人、替班人确认时间、审批人与状态，重复操作不会导致班次被反复交换。流程上线前已提交的申请仍处于待主管审批状态，由主管直接处理。
...
- `GET/POST /api/v1/shift-requests`、`PUT /api/v1/shift-requests/:id/confirm`、`PUT /api/v1/shift-requests/:id/review`：调班申请、替班人确认与主管审批。
